@testset "Testing easy_basis.jl" begin
	@testset "Testing measure_N_up" begin
		...
		counts = map(s -> measure_N_up(s,4),[f,a,b,c,d])
	...
	@testset "Testing measure_N_up_A" begin
	    ...
	    d = convert(UInt64, 15)
	    # display_states([f,a,b,c,d])
	    counts = map(s -> measure_N_up_A(s,4),[f,a,b,c,d])
	...
	@testset "Testing measure_N_up_B" begin
	    ...
	    d = convert(UInt64, 15)
	    # display_states([f,a,b,c,d])
	    counts = map(s -> measure_N_up_B(s,4),[f,a,b,c,d])
	...
	@testset "Testing make_translation_function." begin
		...
		tr_fcn = make_translation_function(10,1,1)
		xx = UInt64(3)
		pf = Complex(1.0)
		(gx,pf2) = tr_fcn(xx,pf)
	...
	@testset "Testing make_spin_flip_function" begin
		Z2_fcn = make_spin_flip_function(5,1)
		Z2_fcn_2 = make_spin_flip_function(5,-1)
		xxx = UInt64(0)
	...
	@testset "Testing make_Inversion_function" begin
		...
		inv_fcn = make_Inversion_function(L,1,2)
		inv_fcn_2 = make_Inversion_function(L,-1,2)
		...
		inv_fcn = make_Inversion_function(L,1,1)
		inv_fcn_2 = make_Inversion_function(L,-1,1)
	...
	@testset "Testing make_easy_orbit_function." begin
		...
		symmetries = Dict("K" => 1)
		orb_fcn = make_easy_orbit_function(L,2,symmetries)
		
		xx = UInt64(1)
		orb = orb_fcn(xx)

		@test orb isa Nullable{ORBIT}
		if ~isnull(orb)
	...
	@testset "Testing easy basis function" begin
	    L = 4
	    SzTrZ2basis = make_basis(L;syms=Dict("SzA" => 2,"K" => 0),unitCellSize=2)
	    ...
	    @test SzTrZ2basis.q_numbers == Dict("K" => 0,"SzA" => 2)
